...
#define FF_PROFILE_AAC_HE   4  //add by lihengz 2013-4-18
...
static av_cold int aac_encode_close(AVCodecContext *avctx)
{
...
//	__android_log_print(ANDROID_LOG_INFO, "tt","fdk aac_encode_close 4");

    return 0;
}

//static FILE *fp=NULL; //test to del
static av_cold int aac_encode_init(AVCodecContext *avctx)
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt","fdk aac channle:%d, profile:%d, samplerate:%d, flags:%x, cutoff:%d",
		//avctx->channels,avctx->profile,avctx->sample_rate,avctx->flags,avctx->cutoff);

    if ((err = aacEncOpen(&s->handle, 0, avctx->channels)) != AACENC_OK) {
        av_log(avctx, AV_LOG_ERROR, "Unable to open the encoder: %s\n",
               aac_get_error(err));
		//__android_log_print(ANDROID_LOG_ERROR, "tt","Unable to open the encoder: %s\n",aac_get_error(err) );
        goto error;
    }
...
//    ff_af_queue_init(avctx, &s->afq);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt","fdk aac init ok,framesize:%d",avctx->frame_size);
//	fp=fopen("/storage/sdcard0/fdk.aac","wb"); //test to del
							...
							unsigned char *frame, int buf_size, void *data)
{
    ...
    if (!data) {
        ...
        in_ptr                   = data;//frame->data[0];
        in_buffer_size           = 2 * avctx->channels * avctx->frame_size;//frame->nb_samples;
        //__android_log_print(ANDROID_LOG_ERROR, "tt","aac_encode_frame: %d",in_buffer_size );
        in_buffer_element_size   = 2;

        in_args.numInSamples     = avctx->channels * avctx->frame_size ;//frame->nb_samples;
        in_buf.numBufs           = 1;
        in_buf.bufs              = &in_ptr;
...
//        return ret;
    ...
    out_ptr                   = frame;//avpkt->data;
    out_buffer_size           = buf_size;//avpkt->size;
    ...
    if ((err = aacEncEncode(s->handle, &in_buf, &out_buf, &in_args,
		...
		//__android_log_print(ANDROID_LOG_ERROR, "tt","Unable to encode frame: %s",aac_get_error(err) );
        return AVERROR(EINVAL);
    }
...
//	fwrite(out_ptr,1,out_args.numOutBytes,fp); //test to del

    return out_args.numOutBytes;//0;
}

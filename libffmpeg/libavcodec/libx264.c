                      ...
                      int orig_bufsize, void *data)
{
    ...
    if (frame) {
		...
		//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_frame is %d",g_264_idr);
        if (g_264_idr) {
           x4->pic.i_type = X264_TYPE_IDR;		   
           g_264_idr = 0;   
		   av_free(x4->sei);
		   x264_encoder_close(x4->enc);
		   //x4->params.i_sps_id = 5;
		   x4->enc = x264_encoder_open(&x4->params);
		   if (x264_encoder_reconfig(x4->enc, &x4->params)) {
		   	  av_log(NULL,AV_LOG_INFO,"reconfigure failed\n"); 			  
		   }
        }
	...
	//__android_log_print(ANDROID_LOG_INFO, "pp", "X264_frame %d",orig_bufsize);

    do {
        bufsize = orig_bufsize;
	//__android_log_print(ANDROID_LOG_INFO, "pp", "X264_frame11 %d,%d",bufsize,frame);
    if (x264_encoder_encode(x4->enc, &nal, &nnal, frame? &x4->pic: NULL, &pic_out) < 0)
        return -1;

    bufsize = encode_nals(ctx, buf, bufsize, nal, nnal, 0);
	//__android_log_print(ANDROID_LOG_INFO, "pp", "X264_frame22 %d,%d",bufsize,frame);
    if (bufsize < 0)
        return -1;
    } while (!bufsize && !frame && x264_encoder_delayed_frames(x4->enc));
	
	//__android_log_print(ANDROID_LOG_INFO, "pp", "X264_frame1 %d",orig_bufsize);
...
static av_cold int X264_init(AVCodecContext *avctx) 
{
    X264Context *x4 = avctx->priv_data;
    int sw,sh;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_init is ");
    ...
    //__android_log_print(ANDROID_LOG_INFO, "jjf", "bit_rate is %d",avctx->bit_rate);
    if (avctx->bit_rate) {
...
//carlzhong 1足?“2?那y車?
    //x4->params.rc.i_bitrate = 600;
	//avctx->bit_rate = x4->params.rc.i_vbv_buffer_size = x4->params.rc.i_vbv_max_bitrate = 0;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_init is %d,%d,%d,%d", x4->x264opts,x4->preset, \
																		  x4->params.rc.i_vbv_buffer_size,x4->params.rc.i_vbv_max_bitrate);
//end
...
#if 1
		//carlzhong  binding para
		//x4->x264opts = opts;

		//avctx->thread_count = 3;
		//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_init is ");
		//end carlzhong
#endif
...
//    x4->params.b_open_gop     = !(avctx->flags & CODEC_FLAG_CLOSED_GOP);
	...
	//x4->params.rc.i_bitrate = avctx->bit_rate/1000;//jjf
#if FF_API_X264_GLOBAL_OPTS
...
//jjf   set CQP mode
#if CQP_OPTIMIZE
            x4->params.rc.i_rc_method   = X264_RC_CQP;
            x4->params.rc.i_qp_constant = 28;
#endif		
	//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_init1 is %d",avctx->bit_rate);

    x4->params.i_sps_id = avctx->i_sps_id;
    x4->enc = x264_encoder_open(&x4->params);
    if (!x4->enc)
        return -1;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "X264_init2 is ");
   ...
   //__android_log_print(ANDROID_LOG_INFO, "jjf", "bit_rate is %d",avctx->bit_rate);//jjf
    return 0;
}

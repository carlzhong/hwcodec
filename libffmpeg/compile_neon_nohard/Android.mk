...
LOCAL_C_INCLUDES+=$(LOCAL_PATH)/../x264/include  $(LOCAL_PATH)/../faac-1.28/include  $(LOCAL_PATH)/../opencoreamr
#LOCAL_STATIC_LIBRARIES := libx264
#LOCAL_LDFLAGS+=$(LOCAL_PATH)/../x264/libx264.a    $(LOCAL_PATH)/../../obj/local/armeabi/libopencore-amrnb.a
...
#carlzhong
AVDEVICE_C_FILES-$(CONFIG_AVDEVICE)         +=  alldevices.c
AVDEVICE_C_FILES-$(CONFIG_LAVFI_INDEV)         +=  lavfi.c
AVDEVICE_C_FILES-$(CONFIG_AVDEVICE)         += avdevice.c

AVDEVICE_C_FILES += $(AVDEVICE_C_FILES-yes)
AVDEVICE_SRC_FILES := $(addprefix libavdevice/, $(sort $(AVDEVICE_C_FILES)))
...
AVCODEC_C_FILES-$(CONFIG_LIBQCOMH264_ENCODER)            += aqcom264enc.cpp
AVCODEC_C_FILES-$(CONFIG_LIBQCOMH264_DECODER)            += aqcom264dec.cpp
AVCODEC_C_FILES-$(CONFIG_LIBSAMSUNGH264_ENCODER)            += asamsung264enc.c
AVCODEC_C_FILES-$(CONFIG_LIBSAMSUNGH264_DECODER)            += asamsung264dec.c
AVCODEC_C_FILES-$(CONFIG_LIBSTAGEFRIGHT_ICS_ENCODER)        += astagefright_264enc_ics.cpp
...
ARMV6-AVCODEC_ARM_C_FILES-$(CONFIG_AC3DSP)            += arm/ac3dsp_armv6.S

AVCODEC_ARM_C_FILES-$(CONFIG_MPEGAUDIODSP)            += arm/mpegaudiodsp_init_arm.c
ARMV6-AVCODEC_ARM_C_FILES-$(CONFIG_MPEGAUDIODSP)      += arm/mpegaudiodsp_fixed_armv6.S
...
ARMV6-AVCODEC_ARM_C_FILES-$(CONFIG_VP8_DECODER)       += arm/vp8_armv6.S               \
                                          arm/vp8dsp_armv6.S
...
AVCODEC_ARM_C_FILES-$(HAVE_ARMV6)                     += arm/dsputil_init_armv6.c      \
                                          ...
                                          $(ARMV6-AVCODEC_ARM_C_FILES-yes)

VFP-AVCODEC_ARM_C_FILES-$(HAVE_ARMV6)                 += arm/fmtconvert_vfp.S          \

AVCODEC_ARM_C_FILES-$(HAVE_ARMVFP)                    += arm/dsputil_vfp.S             \
                                          arm/dsputil_init_vfp.c        \
                                          $(VFP-AVCODEC_ARM_C_FILES-yes)

AVCODEC_ARM_C_FILES-$(HAVE_IWMMXT)                    += arm/dsputil_iwmmxt.c          \
                                          arm/mpegvideo_iwmmxt.c        \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_FFT)                += arm/fft_neon.S                \
                                          arm/fft_fixed_neon.S          \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_MDCT)               += arm/mdct_neon.S               \
                                          arm/mdct_fixed_neon.S         \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_RDFT)               += arm/rdft_neon.S               \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_H264DSP)            += arm/h264dsp_neon.S            \
                                          arm/h264idct_neon.S           \
                                          arm/h264cmc_neon.S            \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_H264PRED)           += arm/h264pred_neon.S           \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_AC3DSP)             += arm/ac3dsp_neon.S

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_DCA_DECODER)        += arm/dcadsp_neon.S             \
                                          arm/synth_filter_neon.S       \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_RV30_DECODER)       += arm/rv34dsp_init_neon.c       \
                                          arm/rv34dsp_neon.S            \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_RV40_DECODER)       += arm/rv34dsp_init_neon.c       \
                                          arm/rv34dsp_neon.S            \
                                          arm/rv40dsp_init_neon.c       \
                                          arm/rv40dsp_neon.S            \
                                          arm/h264cmc_neon.S            \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_VP3_DECODER)        += arm/vp3dsp_neon.S

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_VP5_DECODER)        += arm/vp56dsp_neon.S            \
                                          arm/vp3dsp_neon.S             \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_VP6_DECODER)        += arm/vp56dsp_neon.S            \
                                          arm/vp3dsp_neon.S             \

NEON-AVCODEC_ARM_C_FILES-$(CONFIG_VP8_DECODER)        += arm/vp8dsp_neon.S
...
AVCODEC_ARM_C_FILES-$(HAVE_NEON)                      += arm/dsputil_init_neon.c       \
                                          ...
                                          $(NEON-AVCODEC_ARM_C_FILES-yes)

AVCODEC_ARM_C_FILES += $(AVCODEC_ARM_C_FILES-yes)
 
AVCODEC_ARM_SRC_FILES := $(addprefix libavcodec/, $(sort $(AVCODEC_ARM_C_FILES)))

AVCODEC_X86_C_FILES-$(CONFIG_MLP_DECODER)             += x86/mlpdsp.c
AVCODEC_X86_C_FILES-$(CONFIG_TRUEHD_DECODER)          += x86/mlpdsp.c

YASM-AVCODEC_X86_C_FILES-$(CONFIG_DCT)                += x86/dct32_sse.asm

YASM-AVCODEC_X86_C_FILES-FFT-$(HAVE_AMD3DNOW)         += x86/fft_3dn.c
YASM-AVCODEC_X86_C_FILES-FFT-$(HAVE_AMD3DNOWEXT)      += x86/fft_3dn2.c
YASM-AVCODEC_X86_C_FILES-FFT-$(HAVE_SSE)              += x86/fft_sse.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_FFT)                += x86/fft_mmx.asm                 \
                                          $(YASM-AVCODEC_X86_C_FILES-FFT-yes)

YASM-AVCODEC_X86_C_FILES-$(CONFIG_DWT)                += x86/dwt_yasm.asm

YASM-AVCODEC_X86_C_FILES-$(CONFIG_H264CHROMA)         += x86/h264_chromamc.asm           \
                                          x86/h264_chromamc_10bit.asm

MMX-AVCODEC_X86_C_FILES-$(CONFIG_H264DSP)             += x86/h264dsp_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_H264DSP)            += x86/h264_deblock.asm            \
...
YASM-AVCODEC_X86_C_FILES-$(CONFIG_H264PRED)           += x86/h264_intrapred.asm          \
                                          x86/h264_intrapred_10bit.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_H264PRED)            += x86/h264_intrapred_init.c

MMX-AVCODEC_X86_C_FILES-$(CONFIG_RV30_DECODER)        += x86/rv34dsp_init.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_RV30_DECODER)       += x86/rv34dsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_RV40_DECODER)        += x86/rv34dsp_init.c            \
                                          x86/rv40dsp.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_RV40_DECODER)       += x86/rv34dsp.asm

YASM-AVCODEC_X86_C_FILES-$(CONFIG_VC1_DECODER)        += x86/vc1dsp_yasm.asm

YASM-AVCODEC_X86_C_FILES-$(CONFIG_DIRAC_DECODER)      += x86/diracdsp_mmx.c x86/diracdsp_yasm.asm

MMX-AVCODEC_X86_C_FILES-$(CONFIG_AC3DSP)              += x86/ac3dsp_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_AC3DSP)             += x86/ac3dsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_CAVS_DECODER)        += x86/cavsdsp_mmx.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_DNXHD_ENCODER)       += x86/dnxhd_mmx.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_MPEGAUDIODSP)        += x86/mpegaudiodec_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_MPEGAUDIODSP)       += x86/imdct36_sse.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_PNG_DECODER)         += x86/png_mmx.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_ENCODERS)            += x86/dsputilenc_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_ENCODERS)           += x86/dsputilenc_yasm.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_GPL)                 += x86/idct_mmx.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_LPC)                 += x86/lpc_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_PRORES_LGPL_DECODER)     += x86/proresdsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_PRORES_LGPL_DECODER)      += x86/proresdsp-init.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_PRORES_DECODER)     += x86/proresdsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_PRORES_DECODER)      += x86/proresdsp-init.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_DWT)                 += x86/snowdsp_mmx.c \
                                          x86/dwt.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_V210_DECODER)       += x86/v210.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_V210_DECODER)        += x86/v210-init.c
MMX-AVCODEC_X86_C_FILES-$(CONFIG_VC1_DECODER)         += x86/vc1dsp_mmx.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_VP3_DECODER)        += x86/vp3dsp.asm
YASM-AVCODEC_X86_C_FILES-$(CONFIG_VP5_DECODER)        += x86/vp3dsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_VP5_DECODER)         += x86/vp56dsp_init.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_VP6_DECODER)        += x86/vp3dsp.asm                  \
                                          x86/vp56dsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_VP6_DECODER)         += x86/vp56dsp_init.c
YASM-AVCODEC_X86_C_FILES-$(CONFIG_VP8_DECODER)        += x86/vp8dsp.asm
MMX-AVCODEC_X86_C_FILES-$(CONFIG_VP8_DECODER)         += x86/vp8dsp-init.c
MMX-AVCODEC_X86_C_FILES-$(HAVE_YASM)                  += x86/dsputil_yasm.asm           \
                                          x86/deinterlace.asm             \
                                          x86/fmtconvert.asm              \
                                          x86/h264_qpel_10bit.asm         \
                                          $(YASM-AVCODEC_X86_C_FILES-yes)

MMX-AVCODEC_X86_C_FILES-$(CONFIG_FFT)                 += x86/fft.c

AVCODEC_X86_C_FILES-$(HAVE_MMX)                       += x86/dsputil_mmx.c             \
                                          x86/fdct_mmx.c                \
                                          x86/fmtconvert_mmx.c          \
                                          x86/idct_mmx_xvid.c           \
                                          x86/idct_sse2_xvid.c          \
                                          x86/motion_est_mmx.c          \
                                          x86/mpegvideo_mmx.c           \
                                          x86/simple_idct_mmx.c      \
                                          $(MMX-AVCODEC_X86_C_FILES-yes) 	 

AVCODEC_X86_C_FILES += $(AVCODEC_X86_C_FILES-yes)

AVCODEC_X86_SRC_FILES := $(addprefix libavcodec/, $(sort $(AVCODEC_X86_C_FILES)))
...
AVFORMAT_C_FILES-$(CONFIG_CONCAT_DEMUXER)            += concatdec.c 
...
AVFILTER_C_FILES-$(CONFIG_SWAPUV_FILTER)                 += vf_swapuv.c
...
MMX-AVFILTER_X86_C_FILES-$(CONFIG_YADIF_FILTER)              += x86/yadif.c
MMX-AVFILTER_X86_C_FILES-$(CONFIG_GRADFUN_FILTER)            += x86/gradfun.c
AVFILTER_X86_C_FILES-$(HAVE_MMX) += $(MMX-AVFILTER_X86_C_FILES-yes)
AVFILTER_X86_C_FILES += $(AVFILTER_X86_C_FILES-yes)
AVFILTER_X86_SRC_FILES := $(addprefix libavfilter/, $(sort $(AVFILTER_X86_C_FILES)))

SWSCALE_C_FILES = options.c rgb2rgb.c swscale.c utils.c yuv2rgb.c \
       swscale_unscaled.c

SWSCALE_C_FILES-$(ARCH_BFIN)          +=  bfin/internal_bfin.S     \
                               bfin/swscale_bfin.c      \
                               bfin/yuv2rgb_bfin.c
...
MMX-SWSCALE_C_FILES-$(HAVE_YASM)      +=  x86/input.c              \
                               x86/output.c             \
                               x86/scale.c
...
SWRESAMPLE_C_FILES += swresample.c audioconvert.c resample.c rematrix.c
SWRESAMPLE_SRC_FILES := $(addprefix libswresample/, $(sort $(SWRESAMPLE_C_FILES)))

PROG_C_FILES-$(CONFIG_FFMPEG) += ffmpeg.c cmdutils.c 
PROG_C_FILES-$(CONFIG_FFPROBE) += ffprobe.c cmdutils.c
PROG_C_FILES-$(CONFIG_FFSERVER) += ffserver.c cmdutils.c
PROG_SRC_FILES := $(PROG_C_FILES-yes) 
...
LOCAL_MODULE := libffmpeg_avutil_neon
else
ifeq ($(HAVE_VFP),yes)
LOCAL_MODULE := libffmpeg_avutil_vfp
else
LOCAL_MODULE := libffmpeg_avutil_armv5te
endif
endif
endif

ifeq ($(ARCH_X86),yes)
LOCAL_SRC_FILES += $(AVCODEC_X86_SRC_FILES) \
                   $(AVFILTER_X86_SRC_FILES)
LOCAL_CFLAGS += -fomit-frame-pointer
LOCAL_MODULE := libffmpeg_x86
endif
...
LOCAL_CFLAGS += -std=c99
LOCAL_LDLIBS := -llog -lz
LOCAL_C_INCLUDES+=$(LOCAL_PATH)/../x264/include    $(LOCAL_PATH)/../faac-1.28/include  $(LOCAL_PATH)/../opencoreamr /E/code/qiyi/android/android4.0/hardware/libhardware/include
#LOCAL_STATIC_LIBRARIES := libx264
#LOCAL_LDFLAGS+=$(LOCAL_PATH)/../x264/libx264.a       $(LOCAL_PATH)/../../obj/local/armeabi/libopencore-amrnb.a
...
TARGET_ARCH_ABI := armeabi-v7a
LOCAL_CFLAGS += -march=armv7-a
LOCAL_MODULE := libffmpeg_codec_neon
else
ifeq ($(HAVE_VFP),yes)
LOCAL_MODULE := libffmpeg_codec_vfp
else
LOCAL_MODULE := libffmpeg_codec_armv5te
endif
endif
endif
...
TARGET_ARCH_ABI := armeabi-v7a
LOCAL_CFLAGS += -march=armv7-a
LOCAL_MODULE := libffmpeg_format_neon
else
ifeq ($(HAVE_VFP),yes)
LOCAL_MODULE := libffmpeg_format_vfp
else
LOCAL_MODULE := libffmpeg_format_armv5te
endif
endif
endif
...
#LOCAL_STATIC_LIBRARIES := libx264 
LOCAL_LDFLAGS+=$(LOCAL_PATH)/../x264/libx264.a     $(LOCAL_PATH)/../../obj/local/armeabi/libopencore-amrnb.a  \
#/E/code/qiyi/forwindows/android-ndk-r8b/sources/cxx-stl/gnu-libstdc++/4.6/libs/armeabi-v7a/libsupc++.a

#LOCAL_LDFLAGS+=-L/E/code/qiyi/forwindows/android-ndk-r8b/sources/cxx-stl/gnu-libstdc++/4.6/libs/armeabi-v7a \
#-L/E/code/qiyi/android/android4.0/lib -lstagefright -lmedia -lutils -lbinder -lgnustl_shared #加入链接库的路径
...
TARGET_ARCH_ABI := armeabi-v7a
LOCAL_CFLAGS += -march=armv7-a
LOCAL_WHOLE_STATIC_LIBRARIES := libffmpeg_avutil_neon libffmpeg_codec_neon libffmpeg_format_neon  #只能用LOCAL_WHOLE_STATIC_LIBRARIES
LOCAL_MODULE := libffmpeg_neon_nohard
else 
ifeq ($(HAVE_VFP),yes)
LOCAL_WHOLE_STATIC_LIBRARIES := libffmpeg_avutil_vfp libffmpeg_codec_vfp libffmpeg_format_vfp  #只能用LOCAL_WHOLE_STATIC_LIBRARIES
LOCAL_MODULE := libffmpeg_vfp
else
LOCAL_WHOLE_STATIC_LIBRARIES := libffmpeg_avutil_armv5te libffmpeg_codec_armv5te libffmpeg_format_armv5te  #只能用LOCAL_WHOLE_STATIC_LIBRARIES
LOCAL_MODULE := libffmpeg_armv5te
endif
endif

...
ConfigMode=arm
MODE=armvfp
FLAGS='-mfpu=vfp -mfloat-abi=softfp'
elif [ "$1" = "neon" ];then
ConfigMode=arm
MODE=neon
...
ConfigMode=arm
MODE=armv5te
FLAGS='-march=armv5te -mfloat-abi=soft'
elif [ "$1" = "x86" ];then
ConfigMode=x86
elif [ "$1" = "--help" ];then
...
export ARM_ROOT=/D/sourceCode/android-ndk-r8b
export ARM_PRE=arm-linux-androideabi
TOOLCHAIN=/D/sourceCode/android-ndk-r8b/toolchains/arm-linux-androideabi-4.6/prebuilt/windows
...
--target-os=linux \
--arch=arm \
--cross-prefix=/D/sourceCode/android-ndk-r8b/toolchains/arm-linux-androideabi-4.6/prebuilt/windows/bin/${ARM_PRE}- \
--extra-cflags="${FLAGS} -fPIC" \
--disable-logging \
--enable-gpl \
--enable-version3 \
--enable-nonfree \
...
--enable-sram \
--enable-pthreads \
--extra-ldflags="-L/D/sourceCode/android-ndk-r8b/platforms/android-8/arch-arm/usr/lib \
-nostdlib" \
--enable-${MODE}

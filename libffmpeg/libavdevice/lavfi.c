...
#define FAIL(ERR) { ret = ERR; goto end; }
    ...
    if (!pix_fmts)
        FAIL(AVERROR(ENOMEM));
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header");
    ...
    if (!(lavfi->graph = avfilter_graph_alloc()))
        FAIL(AVERROR(ENOMEM));
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header1");

    if ((ret = avfilter_graph_parse(lavfi->graph, lavfi->graph_str,
                                    &input_links, &output_links, avctx)) < 0)
        FAIL(ret);
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header2");
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header3");
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header4");
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "lavfi_read_header4 ret=%d",ret);
    return ret;
}

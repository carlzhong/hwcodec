                   ...
                   void (*parse_arg_function)(void *, const char*))
{
    ...
    while (optindex < argc) {
        ...
        if (handleoptions && opt[0] == '-' && opt[1] != '\0') {
            ...
            opt++;
            //__android_log_print(ANDROID_LOG_INFO, "tt", "parse_option is  %d,%d ,%s", argc,optindex,argv[optindex-1]);
            if ((ret = parse_option(optctx, opt, argv[optindex], options)) < 0)
            {
                //__android_log_print(ANDROID_LOG_INFO, "tt", "parse_option error is  %d,%s",optindex, argv[optindex]);
                exit_program(1);
            }
            optindex += ret;
        } else {

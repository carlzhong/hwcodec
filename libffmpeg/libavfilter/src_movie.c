...
static av_cold int amovie_init(AVFilterContext *ctx, const char *args, void *opaque)
{
	...
	//carlzhong
	for(int i = 0; i < 2; i++) {
          if(*(audioFileName[i]) == '\0') {
			 strcpy(audioFileName[i],args);
			 __android_log_print(ANDROID_LOG_INFO, "tt", "amovie_get_samples11 is %s,%s, %d",args,audioFileName[i],i);
			 break;
          }
	}
...
static int amovie_get_samples(AVFilterLink *outlink)
{
    ...
    if (!movie->pkt.size && movie->is_done == 1)
        return AVERROR_EOF;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "amovie_get_samples is %s", movie->file_name);

    /* check for another frame, in case the previous one was completely consumed */
    if (!movie->pkt.size) {
        while ((ret = av_read_frame(movie->format_ctx, &pkt)) >= 0 || ret == AVERROR_EOF) {
            // Is this a packet from the selected stream?
			//__android_log_print(ANDROID_LOG_INFO, "tt", "amovie_get_samples %s,%d，%d,%lld,ret=%d", movie->file_name, movie->stream_index, pkt.stream_index, pkt.pts,ret);
            if ((pkt.stream_index != movie->stream_index) && (ret != AVERROR_EOF)) {
                av_free_packet(&pkt);
                continue;
            } else if (ret == AVERROR_EOF) {
            	//carlzhong
            	//__android_log_print(ANDROID_LOG_INFO, "tt", "amovie_get_samples11 is %s,%s", movie->file_name,audioFileName[1]);
			    if (*(audioFileName[1]) != '\0' && !strcmp(audioFileName[1], movie->file_name)) {
				    av_seek_frame(movie->format_ctx, -1, 0, AVSEEK_FLAG_BACKWARD);
					movie->is_done = 0;
					continue;
			    }else
			       break;

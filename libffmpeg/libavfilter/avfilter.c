...
extern int initialized; //carlzhong
...
AVFilter *avfilter_get_by_name(const char *name)
{
    ...
    for (i = 0; registered_avfilters[i]; i++) {
		//__android_log_print(ANDROID_LOG_INFO, "tt", "registered_avfilters[i] = %s",registered_avfilters[i]->name );        
...
int avfilter_open(AVFilterContext **filter_ctx, AVFilter *filter, const char *inst_name)
{
    ...
    *filter_ctx = NULL;
    ...
    if (!filter){
		//__android_log_print(ANDROID_LOG_INFO, "tt", "filter is Null" );
        return AVERROR(EINVAL);
	}
    ret = av_mallocz(sizeof(AVFilterContext));
    if (!ret){
		//__android_log_print(ANDROID_LOG_INFO, "tt", "AVFilterContext mallocz failed" );
        return AVERROR(ENOMEM);
	}

...
{
    int ret;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avfilter 0" );
    if ((ret = avfilter_open(filt_ctx, filt, name)) < 0)
        goto fail;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avfilter 1" );
    if ((ret = avfilter_init_filter(*filt_ctx, args, opaque)) < 0)
        goto fail;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avfilter 2" );
    if ((ret = avfilter_graph_add_filter(graph_ctx, *filt_ctx)) < 0)
        goto fail;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avfilter 3" );
    return 0;

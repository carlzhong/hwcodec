...
void av_dict_free(AVDictionary **pm)
{
    ...
    if (m) {
        while(m->count--) {
			//__android_log_print(ANDROID_LOG_INFO, "tt", "av_dict_free is %s,%s",m->elems[m->count].key,m->elems[m->count].value);
            if(m->elems[m->count].key)
			   av_free(m->elems[m->count].key);
			if(m->elems[m->count].value)
               av_free(m->elems[m->count].value);
			//__android_log_print(ANDROID_LOG_INFO, "tt", "av_dict_free is end");
        }
		//__android_log_print(ANDROID_LOG_INFO, "tt", "av_dict_free is end1 is %d",m->elems);
		if(m->elems)
           av_free(m->elems);
		//__android_log_print(ANDROID_LOG_INFO, "tt", "av_dict_free is end2 is %d,%d",pm,*pm);
    }
    av_freep(pm);
}

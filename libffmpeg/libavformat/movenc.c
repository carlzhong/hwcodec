...
static int mp4_get_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    if (!ff_codec_get_tag(ff_mp4_obj_type, track->enc->codec_id)){
         if(track->enc->codec_id == CODEC_ID_AMR_NB || \
		  track->enc->codec_id == CODEC_ID_NONE) {//carlzhong add       
         //__android_log_print(ANDROID_LOG_INFO, "tt", "mp4_get_codec_tag  is %d,%d", track->enc->codec_id,track->enc->codec_tag);
         tag = MKTAG('s', 'a', 'm', 'r');
		 return tag;
...
static int mov_write_tkhd_tag(AVIOContext *pb, MOVTrack *track, AVStream *st)
{
	...
	AVDictionaryEntry *t; 
	if(t = av_dict_get(st->metadata, "rotate", NULL, 0)) { 
	  // __android_log_print(ANDROID_LOG_INFO, "tt", "display_matrix  is %s",t->value);
       if(!strcmp(t->value,"90")) {
	   	  // __android_log_print(ANDROID_LOG_INFO, "tt", "display_matrix1  is %s,%u,%x",t->value,-65536,-65536);
           avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 65536); /* reserved */
		   avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 0xffff0000); /* reserved */
		   avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 0x0); /* reserved */
		   avio_wb32(pb, 0x40000000); /* reserved */
	   }else if(!strcmp(t->value,"180")){
			avio_wb32(pb, 0xFFFF0000); /* reserved */
			avio_wb32(pb, 0x0); /* reserved */
			avio_wb32(pb, 0x0); /* reserved */
			avio_wb32(pb, 0x0); /* reserved */
...
int ff_mov_write_extradata(AVFormatContext *s)
{
  int ret = 0;
  AVIOContext *pb = s->pb;
  MOVMuxContext *mov = s->priv_data;
  ...
  for (int i=0; i<s->nb_streams; i++){
	   ...
	   if(st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
		   if (st->codec->extradata_size) {
			   ...
			   if(track->vosData)  av_free(track->vosData);
			   //#undef printf
			   //printf("ff_mov_write_extradata is %d\n", track->vosLen);
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    AVDictionaryEntry *t;
    int i, hint_track = 0;
    //__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header  is %d",t);
    if (!s->pb->seekable) {
    ...
    /* Default mode == MP4 */
    mov->mode = MODE_MP4;
   // __android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header1  is %d",t);
    if (s->oformat != NULL) {
   ...
   // __android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header2  is %d",t);
    ...
    if (!mov->tracks)
        return AVERROR(ENOMEM);
	//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header3  is %d",t);
    ...
    for(i=0; i<s->nb_streams; i++){
		...
		//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header33  is %d,%d,%d",track->mode, \
			//             track->enc->codec_id,track->enc->codec_tag);
        if (!track->tag) {
            av_log(s, AV_LOG_ERROR, "track %d: could not find tag, "
                   "codec not currently supported in container\n", i);
            goto error;
        }
		//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header34  is %d",i);
		...
		//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header35  is %d",i);
        if (!track->height)
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header4  is %d",t);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "mov_write_header5  is %d",t);

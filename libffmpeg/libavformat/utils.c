...
AVInputFormat *av_find_input_format(const char *short_name)
{
    ...
    while ((fmt = av_iformat_next(fmt))) {
		//__android_log_print(ANDROID_LOG_INFO, "tt", "av_find_input_format  %s", fmt->name);
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    if ( (s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        ...
        return 0;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avio_open2 startis ,%s,", filename);
...
int avformat_open_input(AVFormatContext **ps, const char *filename, AVInputFormat *fmt, AVDictionary **options)
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input-1 is ,%s,", filename);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input0 is ,%s,", filename);

    if ((ret = av_opt_set_dict(s, &tmp)) < 0)
        goto fail;

	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input1 is ,%s,", filename);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input2 is ,%s,", filename);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input3 is ,%s,%d", filename,s->flags&AVFMT_FLAG_PRIV_OPT);

    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header)
        if ((ret = s->iformat->read_header(s, &ap)) < 0)
            goto fail;

	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_open_input4 is ,%s,", filename);
...
int avformat_write_header(AVFormatContext *s, AVDictionary **options)
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header  is %d",ret);
    ...
    if (s->priv_data && s->oformat->priv_class && *(const AVClass**)s->priv_data==s->oformat->priv_class &&
        ...
        goto fail;
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header1	is %d",ret);
   ...
   // __android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header2  is %d",ret);
    for(i=0;i<s->nb_streams;i++) {
        st = s->streams[i];
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header3	is %d",ret);
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header4	is %d",ret);
    ...
    //__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header5	is %d",ret);
    /* init PTS generation */
    for(i=0;i<s->nb_streams;i++) {
	...
	//__android_log_print(ANDROID_LOG_INFO, "tt", "avformat_write_header6	is %d",ret);
    return 0;
fail:
    av_dict_free(&tmp);
    return ret;
...
//carlzhong
int64_t g_last_video_pts = AV_NOPTS_VALUE;
int64_t g_last_video_dts = AV_NOPTS_VALUE;
int64_t g_last_video_duration = 0;
int64_t input_delay=3;
extern  int g_fast_slicer;
...
int av_interleaved_write_frame(AVFormatContext *s, AVPacket *pkt){
    ...
    for(;;){
		...
		//__android_log_print(ANDROID_LOG_INFO, "tt", "ret1 is %d",ret);

...
//wild boar
#define QUERY_COUNT 20000
typedef struct _URLReadContext{
	URLContext *h;
	unsigned char *buf;
	int size;
	int* running;
...
static inline int retry_transfer_wrapper(URLContext *h, unsigned char *buf, int size, int size_min,
                                         int (*transfer_func)(URLContext *h, unsigned char *buf, int size), int* running)
{
    int ret, len;
    int fast_retries = 5;
...
///wild boar
static void* doURLReadRetry(URLReadContext *para)
{
	*(para->return_l) = retry_transfer_wrapper(para->h, para->buf, para->size, 1, para->h->prot->url_read, para->running);
	*(para->running) = 0;
	return NULL;
}
...
int ffurl_read(URLContext *h, unsigned char *buf, int size)
{
	...
	para.size = size;
	int iRunning = 1;
	para.running = &iRunning;
	para.return_l = &ret;

	//add pthread by wild boar
	pthread_t URLReadRetry;
	pthread_create(&URLReadRetry, NULL, doURLReadRetry, &para);
	...
	pthread_join(URLReadRetry, NULL);
...
int ffurl_read_complete(URLContext *h, unsigned char *buf, int size)
{
    ...
    int running = 1;
	int* para = &running;
    return retry_transfer_wrapper(h, buf, size, size, h->prot->url_read, para);
}
...
int ffurl_write(URLContext *h, const unsigned char *buf, int size)
{
    ...
    int running = 1;
	int* para = &running;
    return retry_transfer_wrapper(h, buf, size, size, h->prot->url_write, para);
}

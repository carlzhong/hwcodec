...
static int mov_read_tkhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    */
    ...
    for (i = 0; i < 3; i++) {
        ...
        int tmp = avio_rb32(pb);           // 2.30 fixed point (not used)
        //__android_log_print(ANDROID_LOG_INFO, "tt", "display_matrix  is %d,%d,%d",display_matrix[i][0],display_matrix[i][1],tmp);
    }
